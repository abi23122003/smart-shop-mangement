...
export default function ProductFormDialog({ open, product, categories, saving, onClose, onSubmit }) {
  ...
  function submit(event) { event.preventDefault(); if (!form.productCode.trim() || !form.productName.trim() || !form.barcode.trim() || form.categoryId === "") return setError("Code, name, barcode, and category are required."); if (form.quantity < 0 || form.purchasePrice < 0 || form.sellingPrice < 0 || form.minimumStock < 0) return setError("Stock and prices cannot be negative."); onSubmit(form); }
  return <Dialog open={open} onClose={() => !saving && onClose()} maxWidth="sm" fullWidth slotProps={{ paper: { component: "form", onSubmit: submit } }}><DialogTitle>{product ? "Edit product" : "Add product"}</DialogTitle><DialogContent><Stack spacing={2} sx={{ pt: 1 }}>{error && <Alert severity="error">{error}</Alert>}
    <TextField label="Product code" required value={form.productCode} onChange={(e) => change("productCode", e.target.value)} /><TextField label="Barcode" required value={form.barcode} onChange={(e) => change("barcode", e.target.value)} />
